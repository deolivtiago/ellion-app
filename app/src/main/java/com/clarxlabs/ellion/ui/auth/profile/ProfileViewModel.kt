package com.clarxlabs.ellion.ui.auth.profile
...
) : AppViewModel<ProfileModel.State, ProfileModel.Event>(ProfileModel.State()) {

    init {
        setState { it.copy(isFormValid = isFormValid()) }
    }

    override fun sendEvent(event: ProfileModel.Event) {
        when (event) {
            is ProfileModel.Event.OnEmailChanged -> onEmailChanged(event.text.trim())
            is ProfileModel.Event.OnFullNameChanged -> onNameChanged(event.text)
            is ProfileModel.Event.OnPasswordChanged -> onPasswordChanged(event.text.trim())
            is ProfileModel.Event.OnPasswordConfirmationChanged -> onConfirmationChanged(event.text.trim())
            is ProfileModel.Event.OnSubmitClicked -> onSubmitClicked(event.navigateTo)
            is ProfileModel.Event.OnPasswordVisibilityClicked -> onPasswordVisibilityClicked()
            is ProfileModel.Event.OnContactClicked -> onNavigate(event.navigateTo)
            is ProfileModel.Event.OnPoliciesClicked -> onNavigate(event.navigateTo)
            is ProfileModel.Event.OnResetPasswordClicked -> onNavigate(event.navigateTo)
            is ProfileModel.Event.OnSignInClicked -> onNavigate(event.navigateTo, AppRoute.SignIn)
            is ProfileModel.Event.OnTermsClicked -> onNavigate(event.navigateTo)
        }
    }
